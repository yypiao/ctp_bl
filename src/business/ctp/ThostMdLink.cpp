...
#pragma comment(lib, "thostmduserapi.lib") //这种写法可以不用专门对项目属性进行设置。
#pragma comment(lib, "thosttraderapi.lib") //这种写法可以不用专门对项目属性进行设置。
...
void CThostMdLink::StartWork()
{
    ...
    char const * pszFlowPath3 = "abc"; // 引用时 const char *
